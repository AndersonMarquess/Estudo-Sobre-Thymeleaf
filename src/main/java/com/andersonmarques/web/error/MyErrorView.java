package com.andersonmarques.web.error;
...
public class MyErrorView implements ErrorViewResolver {
    ...
    public ModelAndView resolveErrorView(HttpServletRequest request, HttpStatus status, Map<String, Object> map) {
        ...
        switch (status.value()) {
            case 404:
                model.addObject("error", "Desculpe, não conseguimos localizar essa página.");
                model.addObject("message", "A url para a página \""+ map.get("path")+"\" não existe.");
                break;
            case 500:
                model.addObject("error", "Ops, estamos com algum problema interno, lamentamos o inconveniente.");
